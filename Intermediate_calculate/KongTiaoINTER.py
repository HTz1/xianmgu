...
class KongTiaoINTER:
    ...
    def __init__(self):
        ...
        self.index = ['空调系统', '空调压缩机', '储液筒', '冷凝器', '蒸发器']
    ...
    def output_loadspectrum(self, vehicle_id='XCT25L5'):
        ...
        self.out['天行驶小时数'] = 0.35 * self.X3
        self.out['天行驶里程KM'] = 0.6 * self.X6
        self.out['年工作小时数'] = 0.35 * self.X3 * self.X2
        self.out['年行驶里程KM'] = 0.6 * self.X6 * self.X2
        self.out['工作年限'] = self.N
        self.out['总工作时间'] = 0.35 * self.X3 * self.X2 * self.N
        self.out['总行驶里程'] = 0.6 * self.X6 * self.X2 * self.N
...
if __name__ == '__main__':
    kongTiaoINTER = KongTiaoINTER()
    print(kongTiaoINTER.out)
