...
class DiPanGongGongINTER:
    ...
    def __init__(self):
        ...
        self.index = ['底盘公共系统', '液压泵', '中心回转体', '电磁阀', '控制器',
                      '继电器', '侧标志灯']
    ...
    def output_loadspectrum(self, vehicle_id='XCT25L5'):
        ...
        # 不同部分
        # 天行驶小时数
        self.out['天行驶小时数']['控制器'] = self.X3
        self.out['天行驶小时数']['继电器'] = 0.01 * self.X3
        self.out['天行驶小时数']['侧标志灯'] = 0.02 * self.X3
        for component in ['底盘公共系统', '液压泵', '中心回转体', '电磁阀']:
            self.out['天行驶小时数'][component] = 0.35 * self.X3

        # 天行驶里程KM
        self.out['天行驶里程KM']['控制器'] = self.X6
        self.out['天行驶里程KM']['继电器'] = 0.8 * self.X6
        self.out['天行驶里程KM']['侧标志灯'] = 0.3 * self.X6
        for component in ['底盘公共系统', '液压泵', '中心回转体', '电磁阀']:
...
if __name__ == '__main__':
    diPanGongGongINTER = DiPanGongGongINTER()
    print(diPanGongGongINTER.out)